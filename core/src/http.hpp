...
class Http final {
  public:
    Http(shared_ptr<core_gen::Http> http_impl, const shared_ptr<SingleThreadTaskRunner> & runner);
    void get(const string& url, function<void(HttpResponse)>);
  private:
    class Request final : public core_gen::HttpCallback {
      public:
    ...
    const shared_ptr<SingleThreadTaskRunner> m_cb_thread;
    const shared_ptr<core_gen::Http> m_http;
};
