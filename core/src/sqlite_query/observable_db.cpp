...
optional<core::sqlite::Row>
ObserveConnection::read_by_id(const string& table_name, int64_t rowid) {
    const Cursor c = _read_cursor(table_name, rowid);
    return c.is_valid() ? c.values() : optional<core::sqlite::Row>{};
}

std::pair<vector<string>, optional<core::sqlite::Row>>
ObserveConnection::read_by_id_with_cols(const string& table_name, int64_t rowid) {
    const Cursor c = _read_cursor(table_name, rowid);
    return {c.column_names(), c.is_valid() ? c.values() : optional<core::sqlite::Row>{}};
}
