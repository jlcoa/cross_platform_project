...
Client::Client(core::Http http_client) : m_http {http_client} {}

// todo error handling?
void
github::get_users(core::Http http, optional<uint64_t> since, function<void(vector<github::User>)> callback) {
    string url = BASE_URL + "/users";
    if (since) {
        url += "?since=" + std_patch::to_string(*since);
    }

    http.get(url, [callback] (core::HttpResponse resp) {
        if (resp.error) {
            return;
        }
